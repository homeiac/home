...
#
# Installation:
#   1. Copy to: /var/lib/vz/snippets/gpu-reset-vm108.sh
#   2. chmod +x /var/lib/vz/snippets/gpu-reset-vm108.sh
#   3. qm set 108 --hookscript local:snippets/gpu-reset-vm108.sh
#
# Logs: /var/log/gpu-reset.log
...
GPU_CHECK_TIMEOUT=120  # max seconds to wait for renderD128
...
handle_gpu_check() {
    ...
    if ! wait_for_guest_agent 60; then
...
case "$PHASE" in
    ...
    post-start)
        ...
        # This allows Proxmox to complete the start operation
        nohup bash -c "
            source /var/lib/vz/snippets/gpu-reset-vm108.sh $VMID gpu-check-background
        " >> "$LOG_FILE" 2>&1 &
