...
class CrucibleConfig:
    ...
    def validate(self) -> None:
        ...
        if self.replication_factor > len(self.storage_sleds):
            raise ValueError(
                f"Replication factor ({self.replication_factor}) cannot exceed number of sleds ({len(self.storage_sleds)})"
            )
    ...
    def to_dict(self) -> Dict[str, Union[str, int, bool, List[Dict[str, Union[str, int]]]]]:
        ...
        return {
            "storage_sleds": [
                {"ip": sled.ip, "hostname": sled.hostname, "ports": sled.ports, "max_regions": sled.max_regions}
