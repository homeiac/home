...
class DevelopmentManager:
    ...
    def configure_zfs_storage(self) -> Dict[str, Any]:
        ...
        if not zfs_check["success"]:
            ...
            create_result = self._run_ssh_command(node, f"zfs create -o mountpoint={mount_point} {dataset}")
        ...
        mkdir_result = self._run_ssh_command(
            node, f"mkdir -p {mount_point}/{{webtop-config,webtop-home,projects,shared}}"
        )
        ...
        chown_result = self._run_ssh_command(node, f"chown -R 101000:101000 {mount_point}/")
        ...
        if not config_check["success"] or lxc_mount not in config_check["stdout"]:
            ...
            # Add mount point
            mount_result = self._run_ssh_command(node, f"pct set {lxc_id} -mp0 {mount_point},mp={lxc_mount},shared=1")
    ...
    def deploy_webtop_container(self) -> Dict[str, Any]:
        ...
        container_check = self._run_ssh_command(
            node, f"pct exec {lxc_id} -- docker ps -a --format '{{{{.Names}}}}' | grep '^webtop$'"
        )
        ...
        if container_check["success"] and "webtop" in container_check["stdout"]:
            ...
            running_check = self._run_ssh_command(
                node, f"pct exec {lxc_id} -- docker ps --format '{{{{.Names}}}}' | grep '^webtop$'"
            )
            ...
            if running_check["success"] and "webtop" in running_check["stdout"]:
                ...
                start_result = self._run_ssh_command(node, f"pct exec {lxc_id} -- docker start webtop")
  ...
  {self.webtop_config['docker_image']}"""
    ...
    def install_development_tools(self) -> Dict[str, Any]:
        ...
        update_result = self._run_ssh_command(
            node, f"pct exec {lxc_id} -- docker exec webtop apt-get update", timeout=60
        )
        ...
        install_result = self._run_ssh_command(
            node, f"pct exec {lxc_id} -- docker exec webtop apt-get install -y {tools_str}", timeout=300
        )
        ...
        verification_commands = ["git --version", "python3 --version", "node --version", "npm --version"]

        verified_tools = []
        for cmd in verification_commands:
            verify_result = self._run_ssh_command(node, f"pct exec {lxc_id} -- docker exec webtop {cmd}")
