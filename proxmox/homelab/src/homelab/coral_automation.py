...
from .coral_models import ActionType, AutomationPlan, ContainerStatus, CoralAutomationError, CoralMode, SystemState
...
class CoralAutomationEngine:
    ...
    def __init__(
        ...
        self.initializer = CoralInitializer(
            coral_init_dir=coral_init_dir or Path.home() / "code", python_cmd=python_cmd
        )
        self.config_manager = LXCConfigManager(
            container_id=container_id, config_path=config_path, backup_dir=backup_dir
        )
    ...
    def analyze_system_state(self) -> SystemState:
        ...
        state = SystemState(coral=coral_device, lxc=lxc_config, frigate_using_tpu=frigate_using_tpu)
    ...
    def _is_frigate_using_tpu(self, lxc_config) -> bool:
        ...
        """
        return lxc_config.status == ContainerStatus.RUNNING and lxc_config.current_dev0 is not None
    ...
    def create_automation_plan(self, state: SystemState) -> AutomationPlan:
        ...
        if state.coral.mode == CoralMode.NOT_FOUND:
            return AutomationPlan(actions=[ActionType.ERROR_ABORT], reason="No Coral TPU device detected", safe=False)

        # Optimal state - no action needed
        if state.coral.is_initialized and state.config_matches_device and state.lxc.has_usb_permissions:
            return AutomationPlan(
                actions=[ActionType.NO_ACTION], reason="System is optimal - Coral initialized and config correct"
            )
        ...
        if state.coral.is_initialized and (not state.config_matches_device or not state.lxc.has_usb_permissions):
        ...
        if state.coral.is_initialized and state.config_matches_device and state.lxc.status == ContainerStatus.STOPPED:
            actions.append(ActionType.RESTART_CONTAINER)
        ...
        plan = AutomationPlan(
            actions=actions, reason=f"Plan: {reason_text}", backup_required=ActionType.UPDATE_CONFIG in actions
        )
    ...
    def _execute_action(self, action: ActionType, dry_run: bool) -> bool:
        ...
        if action == ActionType.INITIALIZE_CORAL:
            ...
            try:
                ...
                success = self.config_manager.update_config(device_path=current_device.device_path, dry_run=dry_run)
                if success:
            ...
            try:
                ...
                if not dry_run:
                    ...
                    if current_device.device_path:
                        coral_accessible = self.config_manager.verify_coral_access(current_device.device_path)
                        if coral_accessible:
