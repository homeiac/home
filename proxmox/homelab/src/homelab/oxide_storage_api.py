...
class OxideStorageAPI:
    ...
    async def disk_create(self, request: DiskCreate) -> Dict[str, Any]:
        ...
        try:
            ...
            # Update state
            disk.state = (
                DiskState.DETACHED if request.disk_source != DiskSource.IMPORTING_BLOCKS else DiskState.IMPORT_READY
            )
...
def create_storage_api(
    project_id: str = "homelab", config_path: Optional[str] = None, enable_mocking: Optional[bool] = None
) -> OxideStorageAPI:
    """
