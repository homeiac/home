...
from homelab.oxide_storage_api import DiskCreate, DiskSource, OxideStorageAPI, SnapshotCreate, create_storage_api
...
class CrucibleVMManager:
    ...
    async def create_vm_with_storage(
        ...
        try:
            ...
            await self._configure_vm_crucible_storage(proxmox, node_name, vmid, disk_id)
            ...
            return {"vm": vm_config, "disk": disk, "status": "success"}
    ...
    async def clone_vm_from_snapshot(
        self, source_vm_name: str, target_vm_name: str, node_name: str, snapshot_name: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if not snapshot_name:
                ...
                snapshot_request = SnapshotCreate(
                    name=snapshot_name, description=f"Snapshot for cloning {source_vm_name}", disk=source_disk_id
                )
    ...
    async def delete_vm_with_storage(
        self, vm_name: str, delete_storage: bool = True, create_final_snapshot: bool = False
    ) -> None:
        """
        ...
        try:
            ...
            if create_final_snapshot:
                ...
                snapshot_request = SnapshotCreate(
                    name=snapshot_name, description=f"Final snapshot before deleting {vm_name}", disk=disk_id
                )
    ...
    async def create_additional_disk(
        self, vm_name: str, disk_name: str, size_gb: int, device_name: str = "scsi1"
    ) -> Dict[str, Any]:
    ...
    async def snapshot_vm_storage(
        self, vm_name: str, snapshot_name: str, include_additional_disks: bool = False
    ) -> List[Dict[str, Any]]:
        ...
        snapshot_request = SnapshotCreate(
            name=f"{snapshot_name}-boot", description=f"Boot disk snapshot for VM {vm_name}", disk=boot_disk_id
        )
    ...
    async def get_vm_status(self, vm_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"vm_name": vm_name, "error": str(e), "configuration": vm_config}
    ...
    async def list_managed_vms(self) -> List[Dict[str, Any]]:
        ...
        for vm_name, config in self.vm_configs.items():
            try:
                ...
                vm_list.append({"vm_name": vm_name, "error": str(e), "configuration": config})
    ...
    async def _configure_vm_crucible_storage(self, proxmox: Any, node_name: str, vmid: int, disk_id: str) -> None:
        ...
        try:
            ...
            proxmox.nodes(node_name).qemu(vmid).config.post(
                scsi0=f"crucible:{disk_id},size={disk_info['size'] // 1024**3}G", ide2="local:cloudinit"
            )
...
async def create_vm_with_crucible_storage(
    vm_name: str, node_name: str, disk_size_gb: int = 50, project_id: str = "homelab", enable_mocking: bool = False
) -> Dict[str, Any]:
    """
...
async def main() -> None:
    ...
    try:
        ...
        result = await manager.create_vm_with_storage(
            vm_name="test-crucible-vm", node_name="still-fawn", disk_size_gb=20
        )
