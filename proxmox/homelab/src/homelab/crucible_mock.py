    ...
    async def _simulate_latency(self) -> None:
        ...
        if self.crucible_config.mock_latency_ms > 0:
            latency = random.uniform(
                self.crucible_config.mock_latency_ms * 0.5, self.crucible_config.mock_latency_ms * 1.5
            )
            await asyncio.sleep(latency / 1000.0)

    async def _check_failure_rate(self) -> None:
        """Simulate random failures based on configured failure rate."""
        if self.crucible_config.mock_failure_rate > 0 and random.random() < self.crucible_config.mock_failure_rate:
...
class MockCrucibleManager:
    ...
    async def create_volume(
        self, volume_id: str, size_bytes: int, replica_count: Optional[int] = None
    ) -> Dict[str, Any]:
        ...
        try:
            for sled in selected_sleds:
                ...
                replicas.append({"sled_ip": sled.config.ip, "region_id": region_id, "port": region_info["port"]})
