...
```bash
proxmox_k3s_manager/  # Project Root
...
```bash
git clone https://github.com/your-repo/proxmox_k3s_manager.git
cd proxmox_k3s_manager
```
...
Create the `.env` file inside `src/proxmox_k3s_manager/` and populate it with:
...
```bash
cd proxmox_k3s_manager
poetry run python -m proxmox_k3s_manager.main | tee proxmox_setup.log
```

## **Important Notes**

- The application **must be run from the project root**, **not** from `src/` or
  `src/proxmox_k3s_manager/`.
- The `.env` file **must be inside** `src/proxmox_k3s_manager/` because the
